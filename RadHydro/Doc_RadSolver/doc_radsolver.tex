...
For each face we now require that $(\RadJ)_c = (\RadJ)_{cn}$ at each node, where $c$ and $cn$ are the cell indices on either side of the face. Before we can do this we need to establish the convention of face ownership. For this we assign a cell ownership of a face based on the indices of the cells sharing the face. Suppose a face is shared by cell $c$ and $cn$; cell $c$ owns that face if $c < cn$. Now for each node $j$ part of the owned face we require that
\beqn 
(\RadJ)_{c,j} &= (\RadJ)_{cn,j^*} \\
(\RadJ)_{c,j} - (\RadJ)_{cn,j^*}  &= 0\\
\eeqn \\
where $(\RadJ)_{c,j}$ is the flux at the $j$-th node of cell $c$ and node $j$ is on the shared face. $(\RadJ)_{cn,j^*}$ is the flux at the $j^*$-th node of cell $cn$, and where $j^*$ maps to the same system index as $j$.
