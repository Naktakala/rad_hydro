...
\newline
\newline
Next we apply a time integration scheme to the radiation momentum equation 
\beqn 
\frac{\tau}{c^2} (\RadF^{n+1} - \RadF^{n}) + 
\sum_t \theta_t \biggr[ \bnabla \dotp \VEFf \RadE + \frac{\sigma_t}{c}\RadF \biggr]^t
&=\sum_t \theta_t   \biggr[ \frac{\sigma_t}{c} \bigr( \RadE \velocity + \VEFf \RadE \dotp \velocity \bigr) \biggr]^t,
\eeqn 
\beqn 
\RadF^{n+1} - \RadF^{n} + 
\sum_t \frac{\theta_t c^2}{\tau} \biggr[ \bnabla \dotp \VEFf \RadE + \frac{\sigma_t}{c}\RadF \biggr]^t
&=\sum_t \theta_t  \biggr[ \frac{\sigma_t c}{\tau} \bigr( \RadE \velocity + \VEFf \RadE \dotp \velocity \bigr) \biggr]^t,
\eeqn 
\beqn 
\RadF^{n+1} - \RadF^{n} + 
\sum_t \biggr[ \mathbf{a}_1^t \RadE^t + a_2^t \RadF^t \biggr]
&= \sum_t \biggr( \{\mathbf{a}_3\}^t  + \{\mathbf{a}_4\}^t \biggr) \dotp (\RadE\velocity)^t
\eeqn 
\beqn 
(1 + a_2^{n+1}) \RadF^{n+1}
=
- \mathbf{a}_1^{n+1} \RadE^{n+1} 
+(1+a_2^n)\RadF^n -  \mathbf{a}_1^n \RadE^n 
+\sum_t \biggr( \{\mathbf{a}_3\}^t  + \{\mathbf{a}_4\}^t \biggr) \dotp (\RadE\velocity)^t
\eeqn
\beqn 
\RadF^{n+1}
=
- \mathbf{a}_5^{n+1} \RadE^{n+1} 
+(1+a_6^n)\RadF^n -  \mathbf{a}_5^n \RadE^n 
+\sum_t \biggr( \{\mathbf{a}_7\}^t  + \{\mathbf{a}_8\}^t \biggr) \dotp (\RadE\velocity)^t
\eeqn
Define
\beqn
\RadF_1^{n+1} =  - \mathbf{a}_5^{n+1} \RadE^{n+1} 
+(1+a_6^n)\RadF^n -  \mathbf{a}_5^n \RadE^n 
\eeqn
\beqn 
\RadF_2^{n+1} =\sum_t \biggr( \{\mathbf{a}_7\}^t  + \{\mathbf{a}_8\}^t \biggr) \dotp (\RadE\velocity)^t
\eeqn 
therefore
\beqn 
\RadF^{n+1}
...
Now if we do operator splitting on the radiation energy equation we get
\beqn 
\tau (\RadE^{n+\half*} - \RadE^n)
+\bnabla \dotp \RadF_2^{n+\half}
&= 0
\eeqn
\beqn 
\tau (\RadE^{n+1} - \RadE^{n+\half*})
+\sum_t \theta_t \biggr[ \bnabla \dotp \RadF_1 \biggr]^t
&=  \sum_t \theta_t \biggr[ \sigma_a c \bigr( aT^4 - \RadE \bigr) \biggr]^t
- \biggr(\frac{\sigma_t}{c} \RadF_0  \dotp \velocity \biggr)^{n+\half}
\eeqn
therefore, with a Crank-Nicolson scheme, 
\beqn 
\tau (\RadE^{n+\half*} - \RadE^n)
+\bnabla \dotp \biggr[ 
\biggr( \{\mathbf{a}_7\}^{n+\half}  + \{\mathbf{a}_8\}^{n+\half} \biggr) \dotp (\RadE\velocity)^{n+\half} 
\biggr]
&= 0
\eeqn
\beqn 
\tau (\RadE^{n+1} - \RadE^{n+\half*})
+\theta_1   \bnabla \dotp \RadF_1^{n+1}
+\theta_2 \bnabla \dotp \RadF_1^n
&=  \theta_1 \biggr[ \sigma_a c \bigr( aT^4 - \RadE \bigr) \biggr]^{n+1} + \theta_2 \biggr[ \sigma_a c \bigr( aT^4 - \RadE \bigr) \biggr]^n
- \biggr(\frac{\sigma_t}{c} \RadF_0  \dotp \velocity \biggr)^{n+\half}
\eeqn
\newline
\newline
Now for the MFEM spatial discretization. Similar to before, we apply the MFEM discretization only to $\RadF_1$
