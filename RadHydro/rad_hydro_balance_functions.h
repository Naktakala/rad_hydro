...
//      return J;
  ...
  class HydroBalanceFunction : public chi_math::NonLinearFunction
  {
  private:
    const double Cv;
    const double gamma;
  ...
  private:
    VecDbl OneEquationSide(const VecDbl& x) const
    {
      ...
      return {rho*u,
              rho*u*u + k2*rho*T,
              0.5*rho*pow(u, 3.0) + k1*rho*T*u};
    }

  public:
    HydroBalanceFunction(double in_Cv, double in_gamma, double in_rho0,
                            double in_T0, double in_u0) :
      Cv(in_Cv), gamma(in_gamma),
      k1(gamma*Cv), k2((gamma-1.0)*Cv),
      LHS(OneEquationSide({in_rho0, in_T0, in_u0}))
