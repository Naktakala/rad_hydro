...
\begin{subequations}
	...
	+  \bnabla \dotp \RadJ^{n+\half} = 
	 \sigma_a^{n+\half} c \biggr( 
	a T^{4,n+\half}   -\RadE^{n+\half} 
	\biggr)
	+ \biggr( \frac{1}{3} \bnabla \RadE \dotp \velocity \biggr)^{n}
	\eeqn
...
Define:
\beqn
k_5 &= \frac{k_1}{\tau \rho^{n+\half} - k_4} \\
k_6 &= \frac{k_3 
	- \tau(\half \rho || \velocity ||^2)^{n+\half} + \tau E^{n*}}{\tau \rho^{n+\half} - k_4}
\eeqn 
\begin{subequations}
	\splitline
	\beqn 
...
\begin{subequations}
	...
	+ k_4 k_5 \bigr)\RadE^{n+\half}
	+  
	 \frac{1}{V_c} \sum_f \AreaVec_f \dotp \mathbf{k}_f^{n+\half} (\RadE_{cn}^{n+\half} - \RadE_c^{n+\half})
	= 
	-k_3 - k_4 k_6
	+ \tau\RadE^{n*}
	\eeqn
...
\begin{subequations}
	...
	\beqn 
	\tau (\RadE^{n+1} - \RadE^{n+\half*}) 
	+ \half \bnabla \dotp  \RadJ^{n+1} + \half \bnabla \dotp \RadJ^{n} = 
	-k_4 e^{n+1} 
	-k_1\RadE^{n+1} 
	-k_3
	\eeqn
...
\frac{1}{V_c} \sum_f \AreaVec_f \dotp \RadJ_f
\eeqn
and
\beqn 
\RadJ_f = \mathbf{k}_f ( \RadE_{cn} - \RadE_c)
\eeqn  
...
\begin{subequations}
	...
	\big( \tau + k_1 + k_4 k_5\big) \RadE^{n+1} 
	+ 
	\frac{1}{2V_c} \sum_f \AreaVec_f \dotp  \mathbf{k}_f^{n+1} ( \RadE_{cn}^{n+1} - \RadE_c^{n+1}) 
	= 
	-k_3
	-k_4 k_6
	+ \tau \RadE^{n+\half*}
...
\newpage
\subsubsection{General Predictor and Corrector phase with $\theta$ factor}
Define:
\beqn
\theta_1 &\in [0,1] \\
\theta_2 &= 1-\theta_1 
