...
void RadHydroSolver::Initialize()
{
  ...
  //======================================== Check grid
  m_grid = chi_mesh::GetCurrentHandler().GetGrid();
  ...
  m_fv = chi_math::SpatialDiscretization_FV::New(m_grid);
  ...
  //                                         during construction
  const size_t num_local_cells = m_grid->local_cells.size();
  for (auto& field_name_vec : m_scalar_fields)
  ...
  auto& CCL = m_scalar_fields.at("cell_char_length");
  {
  ...
  if (field_functions.empty())
  {
                         ...
                         std::vector<double>* field)
    {
      ...
      auto discretization = std::dynamic_pointer_cast<SD>(m_fv);
