...
void chi_radhydro::SolverB_GDCN_MFEM::Execute()
{
    ...
    for (const auto& cell : m_grid->local_cells)
    {
      ...
      for (const auto& face : cell.faces)
      {
        for (int fj=0; fj<face.vertex_ids.size(); ++fj)
        {
          ...
          const int64_t jmap = m_pwl->MapDOFLocal(cell,j);
  ...
  while (true)
  {
    ...
    AssySolveMFEMEnergyExchange(                    //Defined part of rhsolverB
      sim_refs, *m_pwl, m_list_of_Cc_star,
    ...
    //################################################ Corrector
//    chi::log.Log() << "Corrector" << std::endl;
    //=================================== Advection of U and rad_E
...
//      VecDbl b;
//
//      AssembleMFEMEnergySystem(                             //Defined part of rhsolverA
//        sim_refs, *m_pwl, m_list_of_Cc_star,
//        kappa_a_n  , kappa_t_n,                             //Stuff
//        kappa_a_nph, kappa_t_nph,
//        /*tau=*/1/dt,                                       //tau input
...
//        A, b);                                              //Outputs
//
//      chi_math::GaussElimination(A,b,b.size());
//      rad_E_np1 = b;
//
